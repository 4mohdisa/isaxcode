...
export async function fetchAllBlogSlugs(): Promise<BlogPost[]> {
  try {
    const response = await axios.get('https://wp.isaxcode.com/index.php?rest_route=/wp/v2/hustles');
...
export async function fetchAllBlogPosts(): Promise<BlogPost[]> {
  ...
  export async function fetchBlogPostBySlug(slug: string): Promise<BlogPost | null> {
    ...
    try {
      const response = await axios.get(`https://wp.isaxcode.com/wp-json/wp/v2/hustles?slug=${slug}`);
